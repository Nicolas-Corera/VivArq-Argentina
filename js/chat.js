...
let initialChatLoaded = false; // Flag para controlar si ya se cargó el chat inicial
...
function initializeChat() {
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      // Primero cargar la lista de chats SIEMPRE
      await loadChats();

      // Luego manejar los parámetros de URL si existen
      const urlParams = new URLSearchParams(window.location.search);
      const projectId = urlParams.get("project");
      const withUserId = urlParams.get("with");

      if (withUserId && !initialChatLoaded) {
        initialChatLoaded = true;
        if (projectId) {
          await openOrCreateChat(projectId, withUserId);
        } else {
          await openChatWithUser(withUserId);
        }
        // Limpiar la URL después de abrir el chat
        clearUrlParameters();
      }
...
async function loadChats() {
  ...
  contactsListener = onSnapshot(chatsQuery, async (snapshot) => {
    ...
    for (const change of snapshot.docChanges()) {
      ...
      // Verificar si el chat fue eliminado
      if (
        chatData.deletedAt &&
        currentChat &&
        ...
        // Remover el elemento del chat de la lista
      ...
      if (change.type === "removed") {
        ...
        if (currentChat && currentChat.id === chatId) {
          resetChatInterface();
          clearUrlParameters(); // Limpiar URL cuando se elimina el chat actual
        }
...
async function openOrCreateChat(projectId, otherUserId) {
  try {
    ...
    if (!otherUserDoc.exists()) {
      showNotification("El usuario especificado no existe", "error");
      clearUrlParameters();
      return;
    }
    ...
    if (!projectDoc.exists()) {
      showNotification("El proyecto especificado no existe", "error");
      clearUrlParameters();
      return;
    }
    ...
    querySnapshot.forEach((doc) => {
      ...
      if (chatData.participants.includes(otherUserId) && !chatData.deletedAt) {
        chatId = doc.id;
      }
    });
    ...
    showNotification("Error al acceder al chat", "error");
    clearUrlParameters();
  }
}
...
async function openChatWithUser(otherUserId) {
  try {
    ...
    if (existingChat) {
      ...
      // Esperar a que se actualice la lista de chats
      setTimeout(() => {
...
async function openChat(chatId, projectId, otherUserId) {
  ...
  // Marcar como activo el chat seleccionado
...
function loadMessages(chatId) {
  ...
  window.unsubscribeChatMessages = onSnapshot(messagesQuery, (snapshot) => {
    ...
    snapshot.docs.forEach((doc) => {
      ...
      // Normalize timestamp to a proper Date object
      let timestamp;
      if (messageData.timestamp?.toDate instanceof Function) {
        ...
        timestamp = messageData.localTimestamp;
      } else if (messageData.localTimestamp) {
        timestamp = new Date(messageData.localTimestamp);
      } else {
    ...
    // Sort messages by timestamp - now safe since all timestamps are Date objects
    allMessages.sort((a, b) => {
      // Double-check that both timestamps are valid Date objects
...
async function deleteChatConversation() {
  ...
  try {
    ...
    deleteChat.innerHTML = " Eliminar Chat";
  } catch (error) {
